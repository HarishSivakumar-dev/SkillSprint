...
public class SecurityConfig
{
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity sec) throws Exception
	{
		return sec.csrf(csrf->csrf.disable())
				  .authorizeHttpRequests(request->request.requestMatchers("/app/login","/app/register","/app/refresh", "/app/otp/registration/send" , "/app/otp/registration/verify" ).permitAll().anyRequest().authenticated())
