...
public interface UserRepo extends JpaRepository<UserRegistration, Integer>
{
	...
	@Query(value="SELECT COUNT(u) from UserRegistration u JOIN u.roles r WHERE u.joinedDate >=:start AND u.joinedDate< :end AND r.rolename= :rolename AND SIZE(u.roles)=1")
